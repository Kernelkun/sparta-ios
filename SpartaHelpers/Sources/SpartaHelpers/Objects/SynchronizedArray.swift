...
//  
//
//  Created by Yaroslav Babalich on 18.02.2021.
//
...
open class SynchronizedArray<Element> {
    fileprivate let queue = DispatchQueue(label: "com.spartacommodities.sparta.SynchronizedArray", attributes: .concurrent)
...
public extension SynchronizedArray {
    ...
    var indices: Range<Int> {
        var result: Range<Int> = .init(uncheckedBounds: (0, 0))
        queue.sync { result = self.array.indices }
        return result
    }
