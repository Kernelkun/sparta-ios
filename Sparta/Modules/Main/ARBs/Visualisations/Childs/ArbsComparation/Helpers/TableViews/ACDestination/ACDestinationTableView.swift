...
class ACDestinationTableView: UIView {

    // MARK: - UI

    private var titleLabel: UILabel!
    private var model: ArbsComparationPCPUIModel!
    private var lightSetViews: [ACDestinationLightsSetView] = []
        ...
        for (index, arbV) in model.arbsV.enumerated() {
            ...
            _ = UIView().then { view in
                ...
                view.addSubview(identifierView) {
                    ...
                    $0.size.equalTo(CGSize(width: 42, height: 36))
                }
                ...
                self.model.headers.forEach { header in
                    ...
                    _ = ACDestinationLightsSetView(
                        ...
                        view.onTap { [unowned self] view in
                            guard let view = view as? ACDestinationLightsSetView, let arbVValue = view.arbVValue else { return }
